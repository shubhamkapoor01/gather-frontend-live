...
function Chat({ socket, room, name }) {
	...
	const sendMessage = async (e) => {
		...
		const ciphertext = crypto.AES.encrypt(message, room).toString();
		...
		var bytes = crypto.AES.decrypt(messageData.message, messageData.room);
		messageData.message = bytes.toString(crypto.enc.Utf8);	
  ...
  useEffect(() => {
    socket.on('receive message', (data) => {
			var bytes = crypto.AES.decrypt(data.message, data.room);
